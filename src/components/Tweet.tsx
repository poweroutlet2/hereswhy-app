...
function linkFormatter(text: string | null): string | null {
    ...
    const anchor = text
        ? text.replace(rxCommonMarkLink, '<Link href="$3"> $2 </Link>')
        : text
        ;
    return anchor
}
                ...
                <div className='flex items-start'>
                    <div className="profile-pic relative">
                        <Link href={`https://twitter.com/${author.username}`} target="_blank" rel="noopener noreferrer">
                            <Image className='rounded-full h-12 w-12 mr-6 hover:border-2 border-white' src={author.profile_picture_url || ""} alt="author profile pic" height={400} width={400} unoptimized={true} />
                        </Link>
                    </div>
                    <div className="flex-auto w-2/3 lg:w-full">
                        ...
                        <Link href={`https://twitter.com/${author.username}`} target="_blank" rel="noopener noreferrer" className='text-base font-semibold gap-1 font-normal opacity-70 hover:opacity-100 duration-300'>
                            @{author.username}
                        </Link>
                        ...
                        {replies &&
                            <div className="flex flex-row justify-center mt-2 gap-8 text-sm font-medium tracking-wider">
